...
class Database:
    ...
    def edit_ads(self,id, title, text, price, image, phone, u_id, prod_id, date):

        self.cursor.execute(
                f"UPDATE ads SET ad_title=?, ad_text=?, ad_price=?, ad_images=?, ad_phone=?, ad_owner=?, ad_product=?, ad_date=? WHERE id=?;",
                (id, title, text, price, image, phone, u_id, prod_id, date, )
            )
        self.conn.commit()
